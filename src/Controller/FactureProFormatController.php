...
class FactureProFormatController extends AbstractController
{
    ...
    #[Route('/index_valider/', name: 'app_facture_pro_format_index_valided', methods: ['GET'])]
    public function indexValidated(FactureProFormatRepository $factureProFormatRepository): Response
    {
        return $this->render('facture_pro_format/show.html.twig', [
            'facture_pro_formats' => $factureProFormatRepository->findFactureProValidated(),
        ]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // Pour définir la valeur en attente dans le champ statut
            $factureProFormat->setStatut(Statut::BROUILLON);

            // Pour définir dans la facture pro forma dans le détail
            foreach ($factureProFormat->getDetailFactures() as $detail) {
    ...
    {
        if ($this->isCsrfTokenValid('delete' . $factureProFormat->getId(), $request->request->get('_token'))) {
            $entityManager->remove($factureProFormat);
            $entityManager->flush();

            $this->addFlash('success', 'Informations supprimées avec succès.');
        }
    ...
    public function impression(FactureProFormat $factureProFormat, DetailFactureRepository $detailFactureRepository): Response
    {
        return $this->render('facture_pro_format/impressionFactureProFormat.html.twig', [
            'facture_pro_format' => $factureProFormat,
            'detail_factures' => $detailFactureRepository->findDetailFactureByFactureProFormat($factureProFormat)
        ]);
    }
    ...
    {
        if ($security->isGranted('ROLE_VALIDATED_FACTURE')) {
            $factureProFormat->setStatut(Statut::VALIDATED);
            $entityManager->flush();

            $this->addFlash('success', 'La facture pro-forma a été validée avec succès.');
        ...
        if ($factureProFormat->getStatut() !== Statut::EN_ATTENTE) {
            $this->addFlash('warning', 'La facture pro-forma doit être soumise avant la validation.');
    ...
    public function soumission(FactureProFormat $factureProFormat, EntityManagerInterface $entityManager, UrlGeneratorInterface $urlGenerator): RedirectResponse
    {
        ...
        $this->addFlash('success', 'La facture pro-forma a été soumise avec succès.');
        $url = $urlGenerator->generate('app_facture_pro_format_info', ['id' => $factureProFormat->getId()]);
        return new RedirectResponse($url);
    }
    ...
    public function annulation(FactureProFormat $factureProFormat, EntityManagerInterface $entityManager, UrlGeneratorInterface $urlGenerator): RedirectResponse
    {
        ...
        $this->addFlash('success', 'La facture pro-forma a été annulée avec succès.');
        $url = $urlGenerator->generate('app_facture_pro_format_info', ['id' => $factureProFormat->getId()]);
        return new RedirectResponse($url);
    }

    #[Route('/{factureProFormatId}/convert-to-invoice', name: 'app_convert_to_invoice', methods: ['GET', 'POST'])]
    public function convertToInvoice(
        int $factureProFormatId,
        Request $request,
        FactureProFormatRepository $factureProFormatRepository,
        DetailFactureRepository $detailFactureRepository,
        EntityManagerInterface $entityManager
    ): Response {
        $factureProFormat = $factureProFormatRepository->find($factureProFormatId);

        if (!$factureProFormat) {
            throw $this->createNotFoundException('Facture pro forma non trouvée');
        }

        $invoice = (new Facture())
            ->setDate($factureProFormat->getDate())
            ->setReference($factureProFormat->getReference())
            ->setCodeFacture($factureProFormat->getNumeroFacturePro())
            ->setClient($factureProFormat->getClients())
            ->setModePayement($factureProFormat->getModePayement())
            ->setDateExpiration($factureProFormat->getDateEcheance())
            ->setStatut(Statut::BROUILLON);

        $detailsFactureProFormat = $factureProFormat->getDetailFactures();
        foreach ($detailsFactureProFormat as $detail) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $factureProFormat->setStatut(Statut::CONVERTED);

            $entityManager->persist($invoice);
            $entityManager->persist($factureProFormat);
            $entityManager->flush();

            $this->addFlash('success', 'La facture pro-forma a été convertie en facture avec succès.');
            return $this->redirectToRoute('app_facture_pro_format_index_valided', ['id' => $invoice->getId()], Response::HTTP_SEE_OTHER);
        }
